...
| 版本 | 主要更新 |
|------|----------|
| v1.56 | **電信公司簡訊通知系統**：流量警告改為頂部橫幅通知（電信公司短信風格）+ 新增「訊息」App 可查看簡訊。Debug 按鈕改為點擊開啟、流量調整改為 ±100MB |
| v1.55 | 限制文字選取（只有輸入框可選取）+ Debug 浮窗按鈕 |
| v1.54 | 選取手柄背板修復 |
...
當流量低於 500MB 或號碼被封鎖時，不再在聊天中顯示警告訊息，而是：

1. **頂部橫幅通知**：畫面最上方彈出類似手機短信的通知橫幅，5 秒後自動消失
2. **訊息 App**：Dock 欄的「訊息」圖示顯示未讀紅點，點擊進入可查看所有簡訊
3. **簡訊內容**：模擬電信公司短信，包含流量提醒、號碼封鎖通知等
