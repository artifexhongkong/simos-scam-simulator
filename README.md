...
[![Download APK](https://img.shields.io/badge/Download-SimOS_v1.55.apk-blue?style=for-the-badge&logo=android)](https://github.com/artifexhongkong/simos-scam-simulator/releases/download/v1.55/SimOS_v1.55.apk)

> 所有版本：[Releases 頁面](https://github.com/artifexhongkong/simos-scam-simulator/releases)

### 安裝方式
1. 點擊上方按鈕下載 `SimOS_v1.55.apk`
...
| 版本 | 主要更新 |
|------|----------|
| v1.55 | **限制文字選取**（只有輸入框可選取）+ **Debug 浮窗按鈕**（可拖曳，含一鍵詐騙成功/失敗/警覺、DRC/流量/風控/積分調整、解鎖全部 NPC、重置遊戲等） |
| v1.54 | 選取手柄背板修復：`android:background` 設為 `@null`（透明） |
| v1.53 | windowBackground 黑→白（無效） |
...
遊戲內右下角有一個橘色 Debug 按鈕（可拖曳到任意位置），點擊展開除錯面板：
