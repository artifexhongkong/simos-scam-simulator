...
[![Download APK](https://img.shields.io/badge/Download-SimOS_v1.64.apk-blue?style=for-the-badge&logo=android)](https://github.com/artifexhongkong/simos-scam-simulator/releases/download/v1.64/SimOS_v1.64.apk)

> 所有版本：[Releases 頁面](https://github.com/artifexhongkong/simos-scam-simulator/releases)

### 安裝方式
1. 點擊上方按鈕下載 `SimOS_v1.64.apk`
...
| 版本 | 主要更新 |
|------|----------|
| v1.65 | **多次詐騙同一 NPC**（成功後可繼續對話，難度遞增）+ 玩家可開具體金額 + Debug 浮窗不可選取 |
| v1.64 | 移除簡訊「代號維持不變」+ 修復 Debug 文字選取 + 防止成功後二次詐騙（已還原） |
| v1.63 | 電話號碼系統（代號不變）+ 簡訊通知直接進入詳情頁 |
| v1.62 | 長按刪除好友可點空白取消 |
| v1.61 | 修復長按刪除好友 bug + 類型錯誤 |
| v1.60 | 長按刪除好友 + 新號碼 350 DRC + 看廣告免費 + Debug 全域文字選取 |
| v1.59 | 黑網 App + 風控系統完善 |
| v1.58 | 修正 NPC 角色混淆 |
| v1.57 | 全螢幕 iOS 簡訊介面 + SMS 回覆 |
| v1.56 | 電信公司簡訊通知系統 |
| v1.55 | 限制文字選取 + Debug 浮窗按鈕 |
| v1.54 | 選取手柄背板修復 |
| v1.52 | 套用 LinguaVerse 極簡做法 |
| v1.45 | 程序化 NPC 產生器 |
| v1.40 | 基礎版本 |

## 黑網 App（DarkNet）

Dock 欄的「黑網」App（原「瀏覽器」），採用深色瀏覽器 UI 風格：

### 功能
- **免洗號碼商店**：花費 150 DRC 購買新電話號碼
  - 全新代號 + 頭像 + TeleChat ID
  - 風控值重置為 0
  - 保留所有 DRC、流量、積分、情報、好友
  - 購買後收到黑網服務確認簡訊
...
- 返回/前進/重新整理按鈕
- 鎖頭圖示（加密連線）
- 底部「離開黑網」+ Tor Network 標示

## 簡訊系統

### 通知橫幅
- 流量 ≤500MB 或號碼被封時，畫面最上方彈出電信公司簡訊通知
- **點擊橫幅直接進入簡訊聊天頁面**
- 5 秒後自動消失

### 簡訊 App（Dock 欄「訊息」）
- **全螢幕 iOS Messages 風格**：聊天氣泡 + 輸入框
- 可回覆簡訊：
  - 回覆 `YES` / `Y` / `是` / `好` → 購買 2GB 補充包（扣 30 DRC），系統回覆確認
  - 回覆其他文字 → 靜默扣除 100MB 流量（無提示）
- 左上角返回、右上角刪除
